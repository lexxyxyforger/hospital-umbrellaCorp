      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          {STEPS.map((label, i) => (
            <div key={label} className="flex items-center shrink-0">
              <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-all ${i < step ? "gradient-primary text-white" : i === step ? "bg-blue-600 text-white ring-4 ring-blue-100" : "bg-slate-200 text-slate-500"}`}>
                  {i < step ? "✓" : i + 1}
                </div>
        ...
        {step === 1 && selectedDoctor && (
          <div className="space-y-4">
                ...
                {getAvailableDates().map((date) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-xl border text-center transition-all ${selectedDate === date ? "gradient-primary text-white border-transparent" : "border-slate-200 hover:border-blue-300 text-slate-700"}`}
                    >
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {getTimesForDate().map((time) => (
                    <button
                      ...
                      className={`py-2.5 rounded-xl border text-sm font-medium transition-all ${selectedTime === time ? "gradient-primary text-white border-transparent" : "border-slate-200 text-slate-700 hover:border-blue-300"}`}
                    >
                      {time}
                    </button>
                  ))}
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(0)} className="flex-1 py-3 rounded-xl border border-slate-200 text-slate-700 font-medium hover:border-slate-300">Kembali</button>
              <button onClick={() => setStep(2)} disabled={!selectedDate || !selectedTime} className="flex-1 py-3 rounded-xl gradient-primary text-white font-medium disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90">Lanjut</button>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl border border-slate-100 card-shadow p-6 space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Nama sesuai KTP" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 outline-none text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Nomor HP *</label>
                <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="08xx-xxxx-xxxx" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 outline-none text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Email</label>
                <input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} type="email" placeholder="email@example.com" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 outline-none text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Tanggal Lahir</label>
                <input type="date" value={form.dob} onChange={(e) => setForm({ ...form, dob: e.target.value })} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 outline-none text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Jenis Kelamin</label>
                <select value={form.gender} onChange={(e) => setForm({ ...form, gender: e.target.value })} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-blue-400 outline-none text-sm bg-white">
            ...
            <div>
              ...
              <textarea value={form.complaint} onChange={(e) => setForm({ ...form, complaint: e.target.value })} placeholder="Jelaskan keluhan atau gejala yang Anda rasakan..." rows={3} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 outline-none text-sm resize-none" />
              <div className="flex flex-wrap gap-1.5 mt-2">
                {COMPLAINT_SUGGESTIONS.map((s) => (
                  <button key={s} onClick={() => setForm({ ...form, complaint: s })} className="px-2.5 py-1 rounded-full border border-slate-200 text-xs text-slate-600 hover:border-blue-300 hover:text-blue-600 hover:bg-blue-50 transition-all">{s}</button>
                ))}
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1.5">Catatan Tambahan</label>
              <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Riwayat penyakit, alergi obat, dll (opsional)" rows={2} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 outline-none text-sm resize-none" />
            </div>
            <div className="flex gap-3">
              ...
              <button onClick={() => setStep(3)} disabled={!form.name || !form.phone || !form.complaint} className="flex-1 py-3 rounded-xl gradient-primary text-white font-medium disabled:opacity-50">Lanjut</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && selectedDoctor && (
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleSubmit} disabled={isSubmitting} className="flex-1 py-3 rounded-xl gradient-primary text-white font-semibold disabled:opacity-70 flex items-center justify-center gap-2">
                {isSubmitting ? (
